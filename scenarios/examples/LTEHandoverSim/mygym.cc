...
namespace ns3 {
    ...
    MyGymEnv::MyGymEnv(double stepTime, uint32_t N1, uint32_t N2, uint16_t N3,
        ...
        block_Thr = blockThr;                  // Blockage threshold expressed in Mb/s
        m_chooseReward = rewardType;           //0: average overall throughput, 1: PRBs utilization deviation, 2: number of blocked users
        m_interval = stepTime;
        m_cellCount = N1;
        m_userCount = N2;
    ...
    MyGymEnv::calculate_rewards() {
        ...
        for (itr = UserThrouput.begin(); itr != UserThrouput.end(); itr++) {
            ...
            std::cout << "#users : " << tempmap.size() << std::endl;
        ...
        std::string str_tmp = "Number of users in the first " + std::to_string(m_cellCount) + " cells: ";
        for (int k=0; k<int(m_cellCount); k++){
            str_tmp = str_tmp + std::to_string(m_UesNum.at(k)) + " ";
        }
        std::cout << str_tmp << std::endl;
        NS_LOG_LOGIC(str_tmp);
