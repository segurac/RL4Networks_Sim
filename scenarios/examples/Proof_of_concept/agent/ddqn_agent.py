...
Result_row = []
Rew_ActIndx = []
MCS2CQI = np.array([1, 2, 3, 3, 3, 4, 4, 5, 5, 6, 6, 6, 7, 7, 8, 8, 8, 9, 9, 9, 10, 10, 10, 11, 11, 12, 12, 13,
                    14])  # To map MCS indexes to CQI
...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        ...
        for time in range(max_env_steps):
            ...
            action = np.base_repr(action_index + int(a_level) ** int(a_num), base=int(a_level))[
                     -a_num:]  # decoding the action index to the action vector
